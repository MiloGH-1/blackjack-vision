...
- **Tracker (`tracker.py`).** Card datasets label corner indices, so one physical card yields about 2 detections. `merge_corners` clusters same-label detections within `merge_fraction` × frame diagonal. `stable()` supports duplicate labels (multi-deck), keeping k copies of a label if at least `min_hits` of the last `window` frames showed at least k copies. k is capped at `max_copies` (the live app sets it to the deck count), so with 1 deck, corners that failed to merge can't become duplicate cards.
- **Dealer/player split** is purely positional (`hands.split_by_region`). With `layout: left-right` (the default), cards left of `divider` (a fraction of frame width) are the dealer's and cards to the right are the player's. `top-bottom` uses height instead, with the dealer above. The live app rotates frames by `rotate` before detection, so positions are always in the rotated frame. Advice is only produced when exactly one dealer card is visible. Only one player hand is supported.
